...
removeV2Ray() {
    ...
    #卸载Xray脚本
    bash "$GO_SH_URL" --remove -x >/dev/null 2>&1
...
updateProject() {
    ...
    if [[ ${INSTALL_WAY} == 0 ]]; then
        if [[ ! -f "$GO_SH_URL" ]]; then
            colorEcho ${RED} "go.sh not found in the current directory!"
            exit 1
        fi
        bash "$GO_SH_URL" --xray
        if [[ $? -ne 0 ]]; then
            colorEcho ${RED} "Failed to install XRay using go.sh. Please check the script."
            exit 1
        fi
    fi
}
...
installFinish() {
    ...
    if [[ ${INSTALL_WAY} == 0 ]]; then
        ...
        v2ray info
        echo -e "Por favor insira o comando 'v2ray' para gerenciar xray\n"
    fi
}
