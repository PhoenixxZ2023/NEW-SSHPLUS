...
class Server(threading.Thread):
    ...
    def printLog(self, log):
        ...
        try:
            print(log)  # Alterado para função print
        finally:
...
class ConnectionHandler(threading.Thread):
    def __init__(self, socClient, server, addr):
        ...
        self.client_buffer = b''  # Inicializado como bytes
        self.server = server
    ...
    def run(self):
        try:
            ...
            hostPort = self.findHeader(self.client_buffer.decode('utf-8'), 'X-Real-Host')  # Decodificar para string
                    ...
                    try:
                        ...
                        if data:
                            if in_ is self.target:
                                self.client.sendall(data)  # Usar sendall para garantir envio completo
                            else:
